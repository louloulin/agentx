...
use autogen_adapter::AutoGenAdapter;
use python_bridge::PythonBridge;
use config::AutoGenConfig;
use error::{AutoGenError, AutoGenResult};
use group_chat::GroupChatManager;

/// AutoGen插件服务
pub struct AutoGenPlugin {
    /// 插件配置
    config: Arc<RwLock<AutoGenConfig>>,
    /// AutoGen适配器
    adapter: Arc<AutoGenAdapter>,
    /// Python桥接器
    python_bridge: Arc<PythonBridge>,
    /// 群聊管理器
    group_chat_manager: Arc<GroupChatManager>,
    /// 插件元数据
...
impl AutoGenPlugin {
    /// 创建新的AutoGen插件实例
    pub async fn new(config: AutoGenConfig) -> AutoGenResult<Self> {
        info!("🚀 初始化AutoGen插件...");
        ...
        // 创建群聊管理器
        let group_chat_manager = Arc::new(GroupChatManager::new().await?);
        
        // 创建AutoGen适配器
        let adapter = Arc::new(AutoGenAdapter::new(
            python_bridge.clone(),
            group_chat_manager.clone()
        ).await?);
    ...
    /// 启动插件服务器
    pub async fn serve(self, addr: std::net::SocketAddr) -> AutoGenResult<()> {
        info!("🌐 启动AutoGen插件gRPC服务器在 {}", addr);
        ...
        Server::builder()
            ...
            .serve(addr)
            .await
            .map_err(|e| AutoGenError::ServerError(format!("gRPC服务器启动失败: {}", e)))?;
        
        Ok(())
    }
    
    /// 验证Python环境和AutoGen安装
    async fn verify_environment(&self) -> AutoGenResult<()> {
        info!("🔍 验证Python环境和AutoGen安装...");
        ...
        // 检查AutoGen版本
        let autogen_version = self.python_bridge.get_autogen_version().await?;
        info!("   AutoGen版本: {}", autogen_version);
        ...
        for package in required_packages {
            if !self.python_bridge.check_package_installed(package).await? {
                return Err(AutoGenError::EnvironmentError(
...
#[tonic::async_trait]
impl AgentXPlugin for AutoGenPlugin {
    /// 初始化插件
    async fn initialize(
        &self,
        ...
        info!("🔧 初始化AutoGen插件，插件ID: {}", req.plugin_id);
    ...
    async fn shutdown(
        ...
        // 关闭群聊管理器
        if let Err(e) = self.group_chat_manager.shutdown().await {
            warn!("群聊管理器关闭时出现警告: {}", e);
        }
    ...
    async fn register_agent(
        ...
        info!("📝 注册AutoGen Agent: {}", agent_info.name);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter("info,agentx_autogen_plugin=debug")
        .init();
    
    info!("🚀 启动AgentX AutoGen插件");
    
    // 加载配置
    let config = AutoGenConfig::load().await?;
    info!("📋 配置加载完成: {:?}", config);
    
    // 创建插件实例
    let plugin = AutoGenPlugin::new(config.clone()).await?;
    
    // 启动服务器
    let addr = format!("{}:{}", config.host, config.port).parse()?;
    info!("🌐 AutoGen插件将在 {} 上提供服务", addr);
